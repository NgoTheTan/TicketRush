...
function iconFor(type) {
  if (type === 'ORDER_PENDING') return 'pending_actions';
  if (type === 'ORDER_PAID') return 'payments';
...
export default function NotificationCenter() {
  ...
  return (
    <>
      <div ref={rootRef} className="fixed bottom-6 right-6 z-[75]">
        <button
          type="button"
          onClick={toggleOpen}
          className="relative flex h-14 w-14 items-center justify-center rounded-full bg-sky-600 text-white shadow-lg shadow-sky-600/25 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2"
        ...
        {open && (
          <div className="absolute bottom-full right-0 mb-3 w-[min(24rem,calc(100vw-2rem))] overflow-hidden rounded-lg border border-slate-100 bg-white shadow-2xl" role="menu">
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
              <div>
                <p className="text-sm font-bold text-slate-900">
                  {selectionMode ? `${selectedIds.size} đã chọn` : 'Thông báo vận hành'}
                </p>
              ...
              {selectionMode ? (
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="text-xs font-semibold text-sky-600 hover:text-sky-700 disabled:text-slate-300"
            ...
            <div className="max-h-[420px] overflow-y-auto py-1">
              {notifications.length === 0 ? (
                ...
                <button
                  ...
                  className={`flex w-full gap-3 px-4 py-3 text-left transition-colors hover:bg-slate-50 ${notification.read ? 'bg-white' : 'bg-sky-50/70'}`}
                  role="menuitem"
                >
                  {selectionMode && (
                    <span
                      className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border text-[14px] ${
                        selectedIds.has(notification.id) ? 'border-sky-600 bg-sky-600 text-white' : 'border-slate-300 bg-white text-transparent'
                      }`}
                    >
                      <span className="material-symbols-outlined text-[16px] leading-none">check</span>
                    </span>
                  )}
                  <span className={`material-symbols-outlined mt-0.5 text-[20px] ${notification.read ? 'text-slate-400' : 'text-sky-600'}`}>
                  ...
                  {!notification.read && <span className="mt-1 h-2 w-2 rounded-full bg-sky-600" />}
                </button>
              ))}
            </div>
          </div>
      ...
      {popup && (
        <div
          ...
          className="fixed bottom-24 right-6 z-[90] w-[min(23rem,calc(100vw-2rem))] rounded-lg border border-sky-100 bg-white p-4 text-left shadow-2xl transition-transform hover:-translate-y-0.5"
        >
          <div className="flex gap-3">
            <span className="material-symbols-outlined mt-0.5 text-[22px] text-sky-600">{iconFor(popup.type)}</span>
